...
const Btn = ({label,color,busy,onClick}) => {
  ...
  return (
    <button onClick={onClick} disabled={busy} className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-all disabled:opacity-50 cursor-pointer ${colors[color]}`}>
      {busy?'...':label}
    </button>
  )
}
...
useEffect(()=>{
  ...
  const loadNews = async () => setNews(await getNews().catch(()=>[]))
  ...
  const submitAnn = async () => {
    ...
    toast.success('Announcement posted!'); setAnnForm({title:'',message:''}); loadAnns()
  }
  ...
  const submitNews = async () => {
    ...
    toast.success('Article published!'); setNewsForm({title:'',content:'',image_url:'',category:'General'}); loadNews()
  }
  ...
  if (loading||!isAdmin) return <div className="min-h-screen bg-shim-bg flex items-center justify-center"><div className="w-10 h-10 border-2 border-shim-primary border-t-transparent rounded-full animate-spin"/></div>
    ...
    <div className="min-h-screen bg-shim-bg">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-24 pb-16">
              ...
              <div className="space-y-2">
                {users.slice(0,5).map(u=>(
                  <div key={u.id} className="flex items-center gap-3 p-4 bg-shim-card border border-shim-border rounded-xl">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-medium text-shim-text">{u.display_name}</p><p className="text-xs text-shim-muted truncate">{u.email}</p></div>
          ...
          <div>
            <div className="flex gap-3 mb-6">
              <div className="relative flex-1">
                ...
                <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or email..." className="input-base pl-10"/>
              </div>
            ...
            {uLoad?(
              ...
              <div className="space-y-3">
                {filtered.map(u=>(
                  <div key={u.id} className={`p-4 rounded-2xl border transition-all ${u.banned?'border-red-500/30 bg-red-500/5':'border-shim-border bg-shim-card'}`}>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                      ...
                      {u.role!=='admin'&&(
                        <div className="flex flex-wrap gap-2">
                          ...
                          {u.banned
                            ? <Btn label="Unban"     color="green" busy={busy===u.id+'unban'} onClick={()=>act('unban',u.id,'User unbanned')}/>
                            : <Btn label="Ban User"  color="red"   busy={busy===u.id+'ban'}   onClick={()=>{if(window.confirm(`Ban ${u.display_name}?`))act('ban',u.id,'User banned')}}/>}
                        </div>
                      )}
                      {u.role==='admin'&&<span className="text-xs text-shim-gold px-3 py-1.5 rounded-lg border border-shim-gold/30">Admin — No actions</span>}
                    </div>
                  </div>
                ))}
                {filtered.length===0&&<div className="text-center py-12 text-shim-muted">No users found</div>}
              </div>
            )}
          </div>
        )}
        ...
        {tab==='Announcements'&&(
          <div className="space-y-8">
            <div className="glass rounded-2xl border border-shim-border p-6">
              <h2 className="text-base font-semibold text-shim-text mb-5">Post New Announcement</h2>
              <div className="space-y-4">
                <div><label className="block text-sm text-shim-textD mb-2">Title</label><input type="text" value={annForm.title} onChange={e=>setAnnForm(f=>({...f,title:e.target.value}))} placeholder="Announcement title..." className="input-base"/></div>
                <div><label className="block text-sm text-shim-textD mb-2">Message</label><textarea value={annForm.message} onChange={e=>setAnnForm(f=>({...f,message:e.target.value}))} placeholder="Announcement message..." rows={3} className="input-base resize-none"/></div>
                <button onClick={submitAnn} className="btn-primary">📢 Post Announcement</button>
              </div>
            </div>
            <div>
              <h2 className="text-base font-semibold text-shim-text mb-4">Active ({anns.length})</h2>
              {anns.length===0?<p className="text-shim-muted text-sm">No active announcements</p>:(
                <div className="space-y-3">
                  {anns.map(a=>(
                    <div key={a.id} className="p-4 bg-shim-card border border-shim-border rounded-xl flex items-start justify-between gap-4">
                      <div>
                        <p className="font-semibold text-shim-text text-sm">{a.title}</p>
                        <p className="text-shim-textD text-sm mt-1">{a.message}</p>
                        <p className="text-xs text-shim-muted mt-1">by {a.admin_name}</p>
                      </div>
                      <button onClick={()=>removeAnnouncement(a.id).then(loadAnns)} className="text-xs text-red-400 hover:text-red-300 transition-colors flex-shrink-0">Remove</button>
                    </div>
                  ))}
                </div>
              )}
        ...
        {tab==='News'&&(
          <div className="space-y-8">
            <div className="glass rounded-2xl border border-shim-border p-6">
              <h2 className="text-base font-semibold text-shim-text mb-5">Publish News Article</h2>
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm text-shim-textD mb-2">Title</label><input type="text" value={newsForm.title} onChange={e=>setNewsForm(f=>({...f,title:e.target.value}))} placeholder="Article title..." className="input-base"/></div>
                ...
                <div><label className="block text-sm text-shim-textD mb-2">Content</label><textarea value={newsForm.content} onChange={e=>setNewsForm(f=>({...f,content:e.target.value}))} placeholder="Article content..." rows={5} className="input-base resize-none"/></div>
                <button onClick={submitNews} className="btn-primary">📰 Publish Article</button>
              </div>
            </div>
            <div>
                ...
                {news.map(n=>(
                  <div key={n.id} className="flex gap-4 p-4 bg-shim-card border border-shim-border rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-shim-muted mt-1">by {n.author} · {new Date(n.created_at).toLocaleDateString()}</p>
                    </div>
                    <button onClick={()=>deleteNews(n.id).then(loadNews)} className="text-xs text-red-400 hover:text-red-300 transition-colors flex-shrink-0">Delete</button>
                  </div>
                ))}
